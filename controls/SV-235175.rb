...
support staff when allocated audit record storage volume reaches #{input('max_audit_record_storage')} percent of
...
utilization reaching #{input('max_audit_record_storage')} percent, they are unable to plan for storage capacity
expansion.
...
administrator (SA).
  ...
  desc  'rationale', ''
  desc  'check', "
    Review OS, or third-party logging application settings to determine whether
...
appropriate personnel when #{input('max_audit_record_storage')} percent of audit log storage capacity is reached."
  impact 0.5
  tag severity: 'medium'
